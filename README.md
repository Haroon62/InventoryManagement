...
## 🔮 Future Improvements
- **Authentication & Authorization**: Add ASP.NET Core Identity to restrict access to inventory managers.
- **Pagination & Sorting**: Implement pagination on the Product List for handling thousands of SKUs efficiently.
- **Reporting Dashboard**: Add charts (e.g., using Chart.js) to visualize low-stock alerts and movement volume over time.
- **API Endpoints**: Add RESTful API controllers to support mobile apps or external integrations.

---

## 🧑‍💻 Interview Questions I should expect about this project

**1. Why did you use ViewModels instead of passing Database Models directly to the view?**
*Answer:* ViewModels decouple the UI from the database schema. They allow us to format data specifically for the view (like passing an `IsLowStock` boolean), protect sensitive data, and enforce UI-specific validation rules without cluttering the Entity Framework models.

**2. How did you ensure the stock never goes below zero?**
*Answer:* In the `StockMovementService`, before saving an `Out` movement, I dynamically calculate the current stock (`Total In - Total Out`). If the requested output is greater than the available stock, the service rejects the transaction and returns a tuple with an error message, which the Controller displays to the user.

**3. What happens to the stock movements if a product is deleted?**
*Answer:* I implemented a "Soft Delete" mechanism. Instead of deleting the record from the database (which would either break foreign keys or cascade delete historical data), I set an `IsActive` flag to `false`. This hides the product from the UI while perfectly preserving historical audit trails.

**4. Why did you choose to calculate the current stock dynamically rather than storing a `CurrentStock` column in the Product table?**
*Answer:* Storing the total in a column creates a risk of data inconsistency (e.g., if a movement is recorded but the product table update fails). By calculating it dynamically from the single source of truth (the movements table), the stock is mathematically guaranteed to be accurate at all times.

**5. How does Dependency Injection (DI) work in your application?**
*Answer:* In `Program.cs`, I registered `ApplicationDbContext`, `IProductService`, and `IStockMovementService` as Scoped services. When a Controller is instantiated, ASP.NET Core's DI container automatically injects instances of these services into the Controller's constructor. This makes the code highly modular and easily testable.
